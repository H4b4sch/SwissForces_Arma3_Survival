...
_cache= if (count _this > 6) then {_this select 6} else {false};
	...
	if (_mA==0) then {_mAH = 1;_mAN = 0.8;};
	if (_mA==1) then {_mAH = 0;_mAN = 0;};
	if (_mA==2) then {_mAH = 0.8;_mAN = 0.8;};
...
if (!(getmarkercolor _mkr == "colorblack")) then {	
	null = [_mkr,[_aGrps,_aSize],[_bGrps,_bSize],[_cGrps,_cSize],[_dGrps,_eGrps,_fGrps,_fSize],_settings,true] execVM "plugins\eos\core\eos_core.sqf";
