...
if !(VAR_STRUCTCANCELED) then {	
	...
	_null = [_variable, [_pos select 0, _pos select 1], getDir _local] execVM "playerActions\BuildDefense.sqf";
}else{
	...
	switch (_variable) do {
		...
		case "Land_i_Garage_V1_F": {
			[player,"establish_vehicle_refit"] call BIS_fnc_addCommMenuItem
		};
	};
};
