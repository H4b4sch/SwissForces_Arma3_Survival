...
_instanceNo = 0;
{
  ...
  // Patrolling Infantry
  _maxPatrolling = var_PlayerProgress;							// The maximal patrolling infantry is depending on the progress / 2
  if(_maxPatrolling >= 16) then {_maxPatrolling = 16};	// If it's over 12 set down to 12
  ...
  if(var_PlayerProgress >= 6) then {
	  ...
	  if(var_PlayerProgress >= 10) then{ _patrollingMotorized = _patrollingMotorized + 1}; // Over 10 Progress add another one
	};
  ...
  switch(_x select 0) do {
  	case "Airport": {
  		...
  		if(var_numPlayers >= 3) then {
  			_patrollingArmor = _patrollingArmor + 1;							// If >= 3 players add 1
  		};
  ...
  /*diag_log format ["Type: %1, Size: %2, Houses: %3, HousePatrol: %4, Patrols: %5, Vehicles: %6, Armored: %7, Static: %8, Flying: %9",
	...
	_null = [["enemyBase" + str _instanceNo],[_housePatrols,1],[_patrollingInfantry,1],[_patrollingMotorized,2],[_patrollingArmor],[_static],[_patrollingHelicopters,0],[0,0,500,EAST,TRUE]] call EOS_Spawn;
